...
public class SampleWorkflow {
  ...
  public static Pair<RepairCandidate, String> repair(File submittedFile, File referenceSolutionFile,
    ...
    Program submittedProgram = TestUtils.loadProgramByFilePath(
        submittedFile.getAbsolutePath());
    Program referenceProgram = TestUtils.loadProgramByFilePath(
        referenceSolutionFile.getAbsolutePath());
    ...
    try {
      structuralAlignmentResult = na.generateStructuralAlignment(
          referenceProgram, submittedProgram);
    ...
    try {
      ...
      // only one set of complete set of repairs to be output by current
